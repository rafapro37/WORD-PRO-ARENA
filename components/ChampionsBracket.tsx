...
const ChampionsBracket: React.FC<ChampionsBracketProps> = ({
  ...
  // ---- Auto-ajuste (escala pra caber na largura) + tela cheia ----
  useEffect(() => {
    const recalcFit = () => {
      const el = containerRef.current;
      if (!el) return;
      const avail = el.clientWidth - 28; // margem interna
      if (avail <= 0 || totalW <= 0) return;
      const fs = document.fullscreenElement === el;
      let scale = avail / totalW;
      if (fs) {
        // Em tela cheia: considera a altura e permite ampliar para preencher (nítido por ser vetorial)
        const availH = el.clientHeight - 28;
        scale = Math.min(scale, availH / totalH);
        setFitScale(Math.min(2.4, scale));
      } else {
        // Normal: apenas reduz para caber na largura, nunca amplia
        setFitScale(Math.min(1, scale));
      }
    };
    recalcFit();
  ...
  const drawLinks = () => {
    ...
    const k = fitScale || 1; // fator de escala aplicado ao stage
    const realW = stage.offsetWidth;   // tamanho real (não-escalado)
    const realH = stage.offsetHeight;
    svg.setAttribute('width', String(realW));
    svg.setAttribute('height', String(realH));
    ...
    const pts = (key: string) => {
      ...
      return Array.from(col.querySelectorAll('[data-bcard]')).map(c => {
        ...
        // divide por k para voltar ao espaço real do stage
        return { leftX: (r.left - sb.left) / k, rightX: (r.right - sb.left) / k, y: (r.top - sb.top) / k + (r.height / k) / 2 };
      });
    };
  ...
  }, [r32, r16, quarters, semis, finals, accent, textColor, fitScale, isFullscreen]);
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute top-3 right-3 z-[20] flex items-center gap-2 px-3 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider transition-all hover:scale-105"
        style={{ background: `${accent}22`, border: `1.5px solid ${accent}`, color: accent, backdropFilter: 'blur(6px)' }}
        title={isFullscreen ? 'Sair da tela cheia' : 'Tela cheia'}
      >
        {isFullscreen ? (
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3v3a2 2 0 0 1-2 2H3M21 8h-3a2 2 0 0 1-2-2V3M3 16h3a2 2 0 0 1 2 2v3M16 21v-3a2 2 0 0 1 2-2h3"/></svg>
        ) : (
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3M21 8V5a2 2 0 0 0-2-2h-3M3 16v3a2 2 0 0 0 2 2h3M16 21h3a2 2 0 0 0 2-2v-3"/></svg>
        )}
        {isFullscreen ? 'Sair' : 'Tela cheia'}
      </button>

      {/* Wrapper que reserva o espaço já escalado e centraliza */}
      <div className="relative" style={{ width: totalW * fitScale, height: totalH * fitScale }}>
        <div ref={stageRef} className="relative" style={{ width: totalW, height: totalH, transform: `scale(${fitScale})`, transformOrigin: 'top left' }}>
        {/* marca d'água */}
