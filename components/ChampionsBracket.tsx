...
const ChampionsBracket: React.FC<ChampionsBracketProps> = ({
  ...
  // Para um lado com `count` confrontos, distribui proporcionalmente sobre os `halfMax` slots base
  const sideCenters = (count: number): number[] => {
    if (count <= 0) return [];
    if (count >= halfMax) return Array.from({ length: count }, (_, i) => baseCenterSide(i));
    const ratio = halfMax / count;
    return Array.from({ length: count }, (_, i) => {
      const a = Math.floor(i * ratio);
      const b = Math.floor((i + 1) * ratio - 1);
      return (baseCenterSide(a) + baseCenterSide(b)) / 2;
    });
  };

  // Divide uma fase em lado esquerdo / direito, cada lado com seus próprios centros
  const buildSides = (matches: MatchLike[]) => {
    const mid = Math.ceil(matches.length / 2);
    const left = matches.slice(0, mid);
    const right = matches.slice(mid);
    const lc = sideCenters(left.length);
    const rc = sideCenters(right.length);
    return {
      left: left.map((m, i) => ({ match: m, top: lc[i] })),
      right: right.map((m, i) => ({ match: m, top: rc[i] })),
    };
  };

  const built: Record<string, { left: { match: MatchLike; top: number }[]; right: { match: MatchLike; top: number }[] }> = {};
  phaseList.forEach(p => { built[p.key] = buildSides(p.matches); });
  ...
  const totalCols = leftOrder.length * 2 + 1; // lados + centro
  ...
  const Card: React.FC<{ match: MatchLike; phase: string }> = ({ match, phase }) => {
    ...
    return (
      <div data-bcard onClick={() => onMatchClick && onMatchClick(match)}
        ...
        {showHead && (
          <div className="text-center font-black uppercase" style={{ fontSize: 9, letterSpacing: '2px', color: themeColor, background: `${themeColor}1f`, borderBottom: `1px solid ${themeColor}30`, padding: '4px' }}>{phase}</div>
        )}
        <div className="flex flex-col">
          <div className="flex items-center" style={{ gap, padding: rowPad, background: homeWon ? `${themeColor}1a` : 'transparent' }}>
          ...
          <div className="flex items-center" style={{ gap, padding: rowPad, background: awayWon ? `${themeColor}1a` : 'transparent' }}>
            <div className="flex items-center justify-center shrink-0" style={{ width: CREST, height: CREST }}>
        ...
        <div data-bcol="final" className="absolute z-[2]" style={{ left: centerX, width: CARD_W, top: finalCenterY, transform: 'translateY(-50%)' }}>
          ...
          <div className="flex flex-col items-center gap-2 mt-4">
            <div className="relative grid place-items-center" style={{ width: 86, height: 86 }}>
              ...
              {championLogoUrl
                ? <img src={championLogoUrl} className="relative z-[2] object-contain" style={{ width: 50, height: 50, filter: 'drop-shadow(0 0 14px rgba(245,197,66,0.5))' }} alt="" />
                : <Trophy size={44} className="relative z-[2]" style={{ color: '#f5c542', filter: 'drop-shadow(0 0 14px rgba(245,197,66,0.5))' }} />}
            </div>
            <div className="font-black uppercase" style={{ fontSize: 9, letterSpacing: '4px', color: '#f5c542' }}>\u2605 Campe\u00e3o \u2605</div>
            <div className="relative z-[2] text-center rounded-xl px-4 py-2.5" style={{ minWidth: 160, background: 'linear-gradient(180deg, rgba(245,197,66,0.14), rgba(245,197,66,0.04))', border: '1.5px solid #f5c542', boxShadow: '0 0 24px rgba(245,197,66,0.45)' }}>
