...
const TournamentDetails: React.FC<TournamentDetailsProps> = ({ 
  ...
  const handleTeamLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file && activeRosterTeamId) { try { const url = await uploadFile('arena-assets', `team-logos/${activeRosterTeamId}_${Date.now()}`, file); onUpdateTeam(activeRosterTeamId, { logoUrl: url }); toast.success('Escudo enviado!'); } catch (err: any) { toast.error('Falha no envio: ' + (err?.message || 'erro')); } e.target.value = ''; } };
          ...
          {showAwardsModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
                 <div className="bg-brand-surface border border-brand-border w-full max-w-2xl p-6 rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar">
                     ...
                     <div className="space-y-4 mb-6"> {[ { key: 'championId', imgKey: 'champion', label: 'Campeão do Torneio', icon: Trophy, color: 'text-yellow-500' }, { key: 'mvpId', imgKey: 'mvp', label: 'MVP da Temporada', icon: Crown, color: 'text-purple-400' }, { key: 'bestStrikerId', imgKey: 'bestStriker', label: 'Artilheiro (Chuteira de Ouro)', icon: Trophy, color: 'text-yellow-400' }, { key: 'bestMidfielderId', imgKey: 'bestMidfielder', label: 'Melhor Meio-Campo (Maestro)', icon: Zap, color: 'text-blue-400' }, { key: 'bestDefenderId', imgKey: 'bestDefender', label: 'Melhor Defensor (Pitbull)', icon: Shield, color: 'text-red-400' }, { key: 'goldenGloveId', imgKey: 'goldenGlove', label: 'Luva de Ouro (Goleiro)', icon: Hand, color: 'text-orange-400' }, { key: 'bestAssistantId', imgKey: 'bestAssistant', label: 'Líder em Assistências', icon: Target, color: 'text-cyan-400' } ].map((award) => { const trophyImg = (awardSelections.trophyImages as any)?.[award.imgKey]; return ( <div key={award.key} className="bg-brand-surfaceHighlight p-4 rounded-lg border border-brand-border"> <label className={`block text-xs uppercase font-bold mb-2 flex items-center gap-2 ${award.color}`}> <award.icon size={14}/> {award.label} </label> <div className="flex gap-3"> <select value={(awardSelections as any)[award.key] || ''} onChange={(e) => setAwardSelections({ ...awardSelections, [award.key]: e.target.value })} className="flex-1 bg-black border border-brand-border rounded p-2.5 text-brand-text text-sm outline-none focus:border-brand-primary" > <option value="">-- Selecione o Vencedor --</option> {tournament.tournamentType === 'X1' ? teams.map(t => (<option key={t.id} value={t.id}>{getTeamNameAndEscudo(t).name}</option>)) : players.map(p => { const t = teams.find(tm => tm.id === p.teamId); return ( <option key={p.id} value={p.id}>{p.name} - {t?.name} ({p.position})</option> ) })} </select> {/* Upload de imagem do troféu */} <label className="shrink-0 w-12 h-12 rounded-lg border border-dashed border-brand-border bg-black/30 flex items-center justify-center cursor-pointer overflow-hidden hover:border-brand-primary transition-all" title="Enviar imagem do troféu"> {trophyImg ? <img src={trophyImg} className="w-full h-full object-contain"/> : <Upload size={16} className="text-brand-textMuted"/>} <input type="file" hidden accept="image/*" onChange={async e => { const file = e.target.files?.[0]; if (!file) return; if (file.size > 2*1024*1024) { toast.error('Imagem muito grande (máx 2MB).'); return; } try { const url = await uploadFile('arena-assets', `award-trophies/${tournament.id}_${award.imgKey}_${Date.now()}`, file); setAwardSelections((prev: any) => ({ ...prev, trophyImages: { ...(prev.trophyImages||{}), [award.imgKey]: url } })); toast.success('Imagem enviada!'); } catch (err: any) { toast.error('Falha no envio: ' + (err?.message || 'erro')); } e.target.value = ''; }} /> </label> </div> </div> ) })} </div>
